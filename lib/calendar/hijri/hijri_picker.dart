...
import 'hijri_calendar_builders.dart';
  ...
  void _handleModeChanged(DatePickerMode mode) {
    ...
    setState(() {
      ...
      if (_mode == DatePickerMode.day) {
        SemanticsService.announce(_selectedHijriDate.toString(), textDirection);
      } else {
        SemanticsService.announce(_selectedHijriDate.toString(), textDirection);
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    final Dialog dialog = Dialog(child: OrientationBuilder(
      ...
      final Widget header = _DatePickerHeader(
        hSelectedHijriDate: _selectedHijriDate,
        mode: _mode,
...
class _DatePickerHeader extends StatelessWidget {
  const _DatePickerHeader({
    Key? key,
    required this.hSelectedHijriDate,
    required this.mode,
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? dayStyle =
        headerTextTheme.headline4?.copyWith(color: dayColor, fontSize: 14);
    final TextStyle? yearStyle =
        headerTextTheme.subtitle1?.copyWith(color: yearColor, fontSize: 14);
    ...
    final Widget yearButton = IgnorePointer(
      ...
      child: _DateHeaderButton(
        ...
        child: Semantics(
          ...
          child: Text("${hSelectedHijriDate.hYear}", style: yearStyle),
        ),
      ),
    );
    ...
    final Widget dayButton = IgnorePointer(
      ...
      child: _DateHeaderButton(
        ...
        child: Semantics(
          ...
          child: Text("${hSelectedHijriDate.toFormat("dd MMMM yyyy")}",
              style: dayStyle),
        ),
      ),
    );
...
class HijriMonthPicker extends StatefulWidget {
  ...
  HijriMonthPicker({
    ...
    this.builders = const HijriCalendarBuilders(),
        ...
        /*  assert(selectedHijriDate.isAfter(
                firstDate.hYear, firstDate.hMonth, firstDate.hDay) ||
            selectedHijriDate.isAtSameMomentAs(
                firstDate.hYear, firstDate.hMonth, firstDate.hDay)),*/
        super(key: key);
...
class _HijriMonthPickerState extends State<HijriMonthPicker> {
  ...
  void initState() {
    ...
    final int monthPage =
        _monthDelta(widget.firstDate, widget.selectedHijriDate);
  ...
  void didUpdateWidget(HijriMonthPicker oldWidget) {
    ...
    if (!widget.selectedHijriDate.isAtSameMomentAs(
        oldWidget.selectedHijriDate.hYear,
        oldWidget.selectedHijriDate.hMonth,
        oldWidget.selectedHijriDate.hDay)) {
      final int monthPage =
          _monthDelta(widget.firstDate, widget.selectedHijriDate);
      _dayPickerController = PageController(initialPage: monthPage);
      _handleMonthPageChanged(monthPage);
  ...
  Widget _buildItems(BuildContext context, int index) {
    ...
    return HijriDayPicker(
      ...
      selectedHijriDate: widget.selectedHijriDate,
        ...
        children: <Widget>[
          Semantics(
            ...
            child: PageView.builder(
              key: ValueKey<HijriCalendar>(widget.selectedHijriDate),
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; true; i += 1) {
      ...
      if (day < 1) {
        ...
        final bool isSelectedDay = selectedHijriDate.hYear == year &&
            selectedHijriDate.hMonth == month &&
            selectedHijriDate.hDay == day;
        if (isSelectedDay) {
