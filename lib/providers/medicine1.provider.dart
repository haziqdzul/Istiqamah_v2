...
class Medicine1Provider extends ChangeNotifier {
  ...
  Future<void> getMedicine() async {
    final prefs = await SharedPreferences.getInstance();
    name = box.read('medicine1');
    intake = int.parse(box.read('medicine1Intake') ?? "0");
    medSwitch = prefs.getBool('m1${AppUser.instance.user!.uid}') ?? false;
  ...
  Future<void> changeSwitch() async {
    ...
    medSwitch = !medSwitch;
    prefs.setBool('m1${AppUser.instance.user!.uid}', medSwitch);
    notifyListeners();
  }
    ...
    BottomPicker.time(
            ...
            onSubmit: (index) {
              ...
              if (schedule != date) {
                ...
                prefs.setBool('m1${AppUser.instance.user!.uid}', true);
                var num = int.parse(box.read('medicine1Intake'));
                var hr = 0;
                var interval = 0;
                if (num == 2) {
                  interval = 12;
                ...
                prefs.setStringList('medicine1', list);
                getSchedule();
                notifyListeners();
              }
            },
  ...
  Future<void> createMedicineReminderNotification(
    ...
    print('set medicine1 ${intake + 1} notification');
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setBool('firstMed1', false);
  ...
  Future<void> getHadisAndQuranVerse(String code) async {
    print('load medicine1 hadith and quran verse');
    langCode = code;
  ...
  Future<void> cancelNotifications() async {
    for (var i = 500; i < 500 + 30 + intake; i++) {
